    ...
    try:
        ...
        for _ in range(params.simulation_count):
            ...
            for node in simulation.network.nodes:
                node.id = dblogger.insert_and_get_id(node.generate_insert_query(simulation.id))
            
            # 任意の回数Generationを繰り返す
            for generation_count in range(params.generation_limit):

                # Genaerationを登録&GenerationIDを取得
                generation_id = dblogger.insert_and_get_id(f'INSERT INTO generations (simulationid, generation_count) VALUES ({simulation.id},{generation_count});')

                # Connectionsを登録
                for startnode in simulation.network.nodes:
                    for endnode, link in node.neighbors.items():
                        dblogger.insert_and_get_id(f'INSERT INTO connections (GenerationID, StartNodeID, EndNodeID, Pheromone, Width) VALUES ({generation_id},{startnode.id},{endnode.id},{link.feromone},{link.width});')

                # antとinterestの生成
                simulation.ant = Ant(simulation.network.start_node, simulation.network.end_node)
                simulation.interest = Interest(simulation.network.start_node, simulation.network.end_node)
